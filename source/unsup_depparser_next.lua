...
function UDepparser:load_kbestdsbank(path, golddsbank, K, do_sampling)
	...
	local id = 1
	for line in io.lines(path..'.'..self.kbestparser:lower()..'scores') do
...
function UDepparser:one_step_train_temp(net, traindsbank_path, trainkbestdsbank_path, golddevdsbank_path, kbestdevdsbank_path, model_dir)
...
function UDepparser:train_net_in_1iter(net, traindsbank_path, golddevdsbank_path, kbestdevdsbank_path, model_dir)
	...
	print('train net')
	adagrad_config, adagrad_state = net:train_with_adagrad(traindsbank, TRAIN_BATCHSIZE,
										TRAIN_N_EPOCHS_IN_1_ITER, {lambda = TRAIN_LAMBDA, lambda_L = TRAIN_LAMBDA_L}, 
										prefix, adagrad_config, adagrad_state, 
										golddevdsbank_path, kbestdevdsbank_path)
	return net
end
...
function UDepparser:train_only_net(net_struct, traindsbank_path, golddevdsbank_path, kbestdevdsbank_path, model_dir)
	...
	net = IORNN:new(net_struct)
	TRAIN_N_EPOCHS_IN_1_ITER = 20
	self:train_net_in_1iter(net, traindsbank_path, golddevdsbank_path, kbestdevdsbank_path, subdir)
end
...
function UDepparser:rerank(net, kbestdsbank, output, K)
	local K = K or 10000
	local ret = {}
...
function UDepparser:rerank_parallel(net_path, dsbank_path, kbestdsbank_path, output, n_procs)
	...
	execute('split -l '..subn..' -d -a 1 '..kbestdsbank_path..'.'..self.kbestparser..'scores '..dir..'/scores')
	...
	while true do
		local done = true
		for i = 1,n_procs do
			local f = io.open(dir..'/'..i..'-results.done', 'r')
			if f == nil then 
				done = false 
				break
			else 
...
function UDepparser:eval(typ, kbestpath, goldpath, output, K)
	...
	if type(typ) == 'string' then
		...
		local net = typ
		parses,ppl = self:rerank(net, kbestdsbank, kbestpath..'.iornnscores', K)
