...
function IORNN:init_params(input)
	...
	self.bh, index = self:create_weight_vector(self.params, index, dim)
...
function IORNN:create_grad()
	...
	grad.bh, index = self:create_weight_vector(grad.params, index, dim)
...
function IORNN:forward_inside(tree)
	...
	local input = (self.Wh * self.L:index(2, tree.word))
					...
					:add(torch.repeatTensor(self.bh,tree.n_nodes,1):t())
