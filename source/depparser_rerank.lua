...
function Depparser:rerank_oracle(kbestdsbank, golddsbank)
...
function Depparser:rerank(net, kbestdsbank)
	local ret = {}
...
function Depparser:eval(net, kbestpath, goldpath, output)
	...
	print('parsing...')
	--local parses = self:rerank_oracle(kbestdsbank, golddsbank)
	local parses = self:rerank(net, kbestdsbank)
