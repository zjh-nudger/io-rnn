...
function load_gold()
	...
	for line in io.lines(human_score_path) do
		...
		if grammar == 'CCG' then 
			str = '(ba (lex '..comps[3].. ') (fa '..comps[2]..' (lex ' .. comps[4] .. ')))'
			str_lm = '(ba (lex '..comps[3].. ') (fa '..comps[5]..' (lex ' .. comps[4] .. ')))'
		...
		local human_rate = tonumber(comps[6])
		local is_high = comps[7] == 'HIGH'
		...
		if cases[case_id] == nil then
			cases[case_id] = {
						...
						subj_id = vocaDic:get_id(comps[3]),
						obj_id = vocaDic:get_id(comps[4]),
		...
		if vocaDic:get_id(comps[2]) == 1 or vocaDic:get_id(comps[3]) == 1 or vocaDic:get_id(comps[4]) == 1 then
			print(line)
		end
	end
...
function rate_add(case) 
	local sem1 = net.L[{{},{case.verb_id}}] + net.L[{{},{case.subj_id}}]
					+ net.L[{{},{case.obj_id}}]
	local sem2 = net.L[{{},{case.landmark_id}}]:clone()
	return compute_score(sem1, sem2)[1]
end
...
function rate_multiply(case)
	local sem1 = torch.cmul(net.L[{{},{case.verb_id}}], net.L[{{},{case.subj_id}}])
					:cmul(net.L[{{},{case.obj_id}}])
	local sem2 = net.L[{{},{case.landmark_id}}]:clone()
	return compute_score(sem1, sem2)[1]
end
...
function rate_iornn( case )
	...
	local sem2 = net.L[{{},{case.landmark_id}}]
	return compute_score(sem1, sem2)[1]
end
